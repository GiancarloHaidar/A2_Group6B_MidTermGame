...
// Occasionally nudges vx to simulate MS balance disruption.
// Feel: intermittent small stumbles, not continuous drift.
//
// ── Timing ───────────────────────────────────────────────────
const STUMBLE_INTERVAL_MIN = 120;  // frames — earliest a stumble can repeat (~2s at 60fps)
const STUMBLE_INTERVAL_MAX = 300;  // frames — latest  (~5s at 60fps)
//   → Increase both to make stumbles rarer.
//   → Decrease both to make them more frequent.
//
// ── Nudge strength ───────────────────────────────────────────
const STUMBLE_STRENGTH_MIN = 0.4;  // px/frame added to vx (weakest stumble)
const STUMBLE_STRENGTH_MAX = 1.1;  // px/frame added to vx (strongest stumble)
//   → Keep STUMBLE_STRENGTH_MAX well below MOVE_SPEED (3.8) to stay subtle.
//   → Raise to 1.6–2.0 for a more disruptive feel.
//   → Lower to 0.2–0.5 for barely-noticeable jitter.
//
// ── Suppression ──────────────────────────────────────────────
// Stumbles are skipped (but timer still resets) when the player
// is nearly idle, so standing still on a platform is always safe.
const STUMBLE_IDLE_DEADZONE = 0.6; // px/frame — |vx| below this = suppress stumble
//   → Raise to suppress stumbles at more walking speeds.
//   → Lower toward 0 to let stumbles fire even when barely moving.
